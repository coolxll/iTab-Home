...
export const WeatherCard: React.FC<WeatherCardProps> = ({ city = '浦东新区' }) => {
  const renderWeatherIcon = (iconName: string) => {
    switch (iconName) {
      case 'sun':
        return <Sun className="w-3.5 h-3.5 text-yellow-300 fill-yellow-300" />
      case 'cloud-rain':
  ...
  return (
    <div className="flex flex-col items-center w-full group">
      {/* Card container */}
      <div className="w-full h-[140px] rounded-2xl p-3.5 bg-gradient-to-br from-[#1E58C8]/90 via-[#2778E2]/90 to-[#3BA2F6]/90 backdrop-blur-xl shadow-xl border border-white/20 text-white flex flex-col justify-between transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] cursor-pointer">
        {/* Top header row */}
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center space-x-1 opacity-95">
        ...
        <div className="grid grid-cols-6 gap-1 pt-1.5 border-t border-white/20 text-center">
          {DEFAULT_WEATHER_FORECAST.map((item, idx) => (
            <div key={idx} className="flex flex-col items-center">
              <span className="text-[10px] text-white/80">{item.day}</span>
              <div className="my-0.5">{renderWeatherIcon(item.icon)}</div>
              <span className="text-[9px] text-white/90 font-light">{item.tempRange}</span>
            </div>
          ))}
        </div>
      </div>
