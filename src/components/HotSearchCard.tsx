...
export const HotSearchCard: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center w-full group">
      {/* Card container */}
      <div className="w-full h-[140px] rounded-2xl p-3 bg-black/45 backdrop-blur-xl border border-white/10 shadow-xl text-white flex flex-col justify-between transition-all duration-300 hover:shadow-2xl hover:scale-[1.02]">
        ...
        <div className="flex flex-col justify-between flex-1 pt-1.5 space-y-1">
          {currentList.map((item) => (
            ...
            >
              ...
              <span className="text-[10px] text-white/40 flex-shrink-0 font-mono">{item.heat}</span>
            </div>
          ))}
        </div>
      </div>
