...
export const HolidayCard: React.FC = () => {
  return (
    <div className="flex flex-col items-center w-full group">
      {/* Card container */}
      <div className="w-full h-[140px] rounded-2xl p-3 bg-black/45 backdrop-blur-xl border border-white/10 shadow-xl text-white flex flex-col justify-between transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] cursor-pointer hover:bg-black/50">
          ...
          {DEFAULT_HOLIDAYS.map((holiday, idx) => (
            <div key={idx} className="flex items-center justify-between border-b border-white/5 pb-1 last:border-0 last:pb-0">
              <div className="flex flex-col">
                <span className="text-xs font-medium text-white/90 leading-tight">{holiday.name}</span>
